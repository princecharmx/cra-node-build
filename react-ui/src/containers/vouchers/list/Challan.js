...
const AddChallanVoucher = Loadable({
  loader: () => import('../add/AddChallanContainer'),
  loading() {
...
class ListSalesVoucher extends Component {
  ...
  getChallanVouchersData() {
    let endDate = formatDate(moment().endOf('day')._d);
    let startDate = this.props.vouchers.getDate.startDate;

    api
      .fetchVouchersList(this.state.companyID, startDate, endDate, 'challan')
      .then(data => {
        let unpaidObj = find(data, item => item._id === 'unpaid' || item._id === 'in-progress');
        let paidObj = find(data, item => item._id === 'paid' || item._id === 'ready');
  ...
  render() {
    ...
    return (
      <Container>
        ...
        <AddChallanVoucher accountNames={this.state.accountNames} />
      </Container>
    );
  }
}
...
const mapStateToProps = state => {
  const { vouchersUpdateState, voucherTypesUpdateState, vouchers, currentCompany } = state;
  return {
    vouchersUpdateState,
